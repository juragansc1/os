...
echo -e "$green                $NC"                                                                                                                          
echo -e "$green♥ TERIMAKSIH TELAH MEMAKAI SCRIPT AIPTUNNEL ♥$NC"
sleep 2
sleep 5
echo -e  "${green}┌──────────────────────────────────────────┐${NC}"
...
echo "AIPTUNNEL" > /etc/xray/username
echo ""
clear
author=$name
echo ""
...
MYIP=$(curl -sS ipv4.icanhazip.com)
IZIN=$(curl -sS https://raw.githubusercontent.com/juragansc1/izinvps/main/ipx | grep $MYIP | awk '{print $3}' )
d1=$(date -d "$IZIN" +%s)
d2=$(date -d "$today" +%s)
EXP=$(( (d1 - d2) / 86400 ))
